...
void PSOldGen::initialize_work(const char* perf_data_name, int level) {
  ...
  printf("old generation created +madvise\n");
  madvise(virtual_space()->low(), _virtual_space->high()-_virtual_space->low(), MADV_HUGEPAGE);
  ...
  printf("> old generation created (/hotspot/vm/gc_impl/ps/psOldGen.cpp)\n");
